#!/usr/bin/env python3
# Copyright (c) 2014-2022 The marsbux Core developers
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'marsbux.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        marsbuxd=os.path.join(bin_dir, 'marsbuxd'),
        marsbux_cli=os.path.join(bin_dir, 'marsbux-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
