#!/usr/bin/env python3
# Copyright (c) 2017-2019 The marsbux Core developers
...
class ChainstateWriteCrashTest(marsbuxTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If marsbuxd exits, then try again.  wait_for_node_exit()
                # should raise an exception if marsbuxd doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, marsbuxd isn't coming back up on restart.  Could be a
        # bug in marsbuxd, or we've gotten unlucky with our dbcrash ratio --
