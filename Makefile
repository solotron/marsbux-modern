...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/marsbux-config.h.in \
...
ACLOCAL = ${SHELL} '/home/solotron/Downloads/marsbux-modern/build-aux/missing' aclocal-1.16
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
ARM_CRC_CXXFLAGS = 
AUTOCONF = ${SHELL} '/home/solotron/Downloads/marsbux-modern/build-aux/missing' autoconf
AUTOHEADER = ${SHELL} '/home/solotron/Downloads/marsbux-modern/build-aux/missing' autoheader
AUTOMAKE = ${SHELL} '/home/solotron/Downloads/marsbux-modern/build-aux/missing' automake-1.16
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} '/home/solotron/Downloads/marsbux-modern/build-aux/missing' makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
ZMQ_LIBS = -lzmq 
abs_builddir = /home/solotron/Downloads/marsbux-modern
abs_srcdir = /home/solotron/Downloads/marsbux-modern
abs_top_builddir = /home/solotron/Downloads/marsbux-modern
abs_top_srcdir = /home/solotron/Downloads/marsbux-modern
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/solotron/Downloads/marsbux-modern/build-aux/install-sh
...
pkgconfig_DATA = libmarsbuxconsensus.pc
marsbuxD_BIN = $(top_builddir)/src/$(marsbux_DAEMON_NAME)$(EXEEXT)
marsbux_QT_BIN = $(top_builddir)/src/qt/$(marsbux_GUI_NAME)$(EXEEXT)
marsbux_CLI_BIN = $(top_builddir)/src/$(marsbux_CLI_NAME)$(EXEEXT)
marsbux_TX_BIN = $(top_builddir)/src/$(marsbux_TX_NAME)$(EXEEXT)
marsbux_WALLET_BIN = $(top_builddir)/src/$(marsbux_WALLET_TOOL_NAME)$(EXEEXT)
marsbux_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win64-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = marsbux-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/marsbux.icns
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/marsbux.ico \
...
COVERAGE_INFO = $(COV_TOOL_WRAPPER) baseline.info \
  test_marsbux_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_marsbux_coverage.info test_marsbux.info fuzz.info fuzz_filtered.info fuzz_coverage.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/marsbux.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/marsbux-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) $(DIST_CONTRIB) $(WINDOWS_PACKAGING) \
	...
	test/util/marsbux-util-test.py \
	test/util/data/marsbux-util-test.json \
...
CLEANFILES = $(OSX_DMG) $(marsbux_WIN_INSTALLER)
...
src/config/marsbux-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/marsbux-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/marsbux-config.h
$(top_srcdir)/src/config/marsbux-config.h.in:  $(am__configure_deps) 
...
$(marsbux_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(marsbuxD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(marsbux_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(marsbux_CLI_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(marsbux_TX_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(marsbux_WALLET_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/marsbux.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/marsbux-Qt: all-recursive
	$(MKDIR_P) $(@D)
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM)  $(marsbux_QT_BIN) $@
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/marsbux-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/marsbux-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
#deploy: $(marsbux_WIN_INSTALLER)

$(marsbux_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(marsbuxD_BIN): FORCE
	$(MAKE) -C src $(@F)

$(marsbux_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)

$(marsbux_TX_BIN): FORCE
	$(MAKE) -C src $(@F)

$(marsbux_WALLET_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_marsbux.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_marsbux -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_marsbux_filtered.info: test_marsbux.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_marsbux_filtered.info
...
#test_marsbux_coverage.info: baseline_filtered.info test_marsbux_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_marsbux_filtered.info -o $@

#total_coverage.info: test_marsbux_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_marsbux_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#fuzz.coverage/.dirstamp: fuzz_coverage.info
#	$(GENHTML) -s $(LCOV_OPTS) $< -o $(@D)
#	@touch $@

#test_marsbux.coverage/.dirstamp:  test_marsbux_coverage.info
#	$(GENHTML) -s $(LCOV_OPTS) $< -o $(@D)
#	@touch $@
...
#cov: test_marsbux.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_marsbux.coverage/ total.coverage/ fuzz.coverage/ test/tmp/ cache/ $(OSX_APP)
