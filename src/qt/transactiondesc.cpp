...
QString TransactionDesc::toHTML_Amounts(interfaces::Wallet& wallet, const interfaces::WalletTx& wtx, const interfaces::WalletTxStatus& status, int unit)
{
    ...
    if (status.blocks_to_maturity > 0 && wtx.credit == 0) // Blocks to maturity covers coinbase and pegouts (HogEx)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += marsbuxUnits::formatHtmlWithUnit(unit, nUnmatured) + " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const interfaces::WalletTxOut& txout : wtx.outputs) {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if (toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            for (const PegOutCoin& pegout : wtx.pegouts) {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, -pegout.GetAmount()) + "<br>";
                if (toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, pegout.GetAmount()) + "<br>";
            }

            if (fAllToMe) {
                // Payment to self
                CAmount nValue = wtx.credit - wtx.change;
                strHTML += "<b>" + tr("Credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, wtx.credit) + "<br>";
                strHTML += "<b>" + tr("Change") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, wtx.change) + "<br>";
                strHTML += "<b>" + tr("Total debit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = wtx.fee;
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        } else {
            //
            ...
            for (const CTxInput& txin : wtx.inputs) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin, ISMINE_ALL)) + "<br>";
                }
            }
            ...
            for (const interfaces::WalletTxOut& out : wtx.outputs) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, wallet.getCredit(out.txout, ISMINE_ALL)) + "<br>";
                }
            }
    ...
    strHTML += "<b>" + tr("Net amount") + ":</b> " + marsbuxUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    return strHTML;
}
...
QString TransactionDesc::toHTML_OrderForm(const interfaces::WalletOrderForm& orderForm)
{
    ...
    // Message from normal marsbux:URI (marsbux:123...?message=example)
...
QString TransactionDesc::toHTML_Debug(interfaces::Node& node, interfaces::Wallet& wallet, const interfaces::WalletTx& wtx, WalletTxRecord* rec, int unit)
{
    ...
    for (const CTxInput& txin : wtx.inputs) {
        ...
        if (get_prev_out(txin, prevout)) {
            ...
            strHTML = strHTML + " " + tr("Amount") + "=" + marsbuxUnits::formatHtmlWithUnit(unit, wallet.getValue(prevout));
        }
    ...
    for (const interfaces::WalletTxOut& txout : wtx.outputs) {
        ...
        strHTML = strHTML + " " + tr("Amount") + "=" + marsbuxUnits::formatHtmlWithUnit(unit, wallet.getValue(txout.txout));
