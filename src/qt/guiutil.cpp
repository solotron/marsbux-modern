...
#include <qt/marsbuxaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a marsbux address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new marsbuxAddressEntryValidator(parent));
    widget->setCheckValidator(new marsbuxAddressCheckValidator(parent));
}

bool parsemarsbuxURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no marsbux: URI
    if(!uri.isValid() || uri.scheme() != QString("marsbux"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!marsbuxUnits::parse(marsbuxUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsemarsbuxURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsemarsbuxURI(uriInstance, out);
}

QString formatmarsbuxURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("marsbux:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(marsbuxUnits::format(marsbuxUnits::BTC, info.amount, false, marsbuxUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openmarsbuxConf()
{
    ...
    /* Open marsbux.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "marsbux.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "marsbux (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("marsbux (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for marsbux*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "marsbux.desktop";
    return GetAutostartDir() / strprintf("marsbux-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a marsbux.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=marsbux\n";
        else
            optionFile << strprintf("Name=marsbux (%s)\n", chain);
