 ...
 * https://github.com/marsbux/marsbux/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //marsbux-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", marsbux_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\marsbux
    // macOS: ~/Library/Application Support/marsbux
    // Unix-like: ~/.marsbux
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "marsbux";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/marsbux";
#else
    // Unix-like
    return pathRet / ".marsbux";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure marsbux Core copyright is not removed by accident
    if (copyright_devs.find("marsbux Core") == std::string::npos) {
        if (strPrefix.find("2011") != std::string::npos) {
            std::string strYear = strPrefix;
            strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
            strCopyrightHolders += "\n" + strYear + "The marsbux Core developers";
        }
    }
