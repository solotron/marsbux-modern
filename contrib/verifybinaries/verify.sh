...
###   marsbuxcore.org and marsbux.org and compares them.
...
WORKINGDIR="/tmp/marsbux_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://marsbuxcore.org"
HOST2="https://marsbux.org"
BASEDIR="/bin/"
VERSIONPREFIX="marsbux-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'marsbux-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "marsbux.org failed to provide signature file, but marsbuxcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "marsbux.org and marsbuxcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the marsbux Core binary release signing key installed?"
   fi
