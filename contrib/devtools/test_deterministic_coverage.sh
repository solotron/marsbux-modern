...
TEST_marsbux_BINARY="src/test/test_marsbux"
...
if [[ ! -e ${TEST_marsbux_BINARY} ]]; then
    echo "Error: Executable ${TEST_marsbux_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_marsbux_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
