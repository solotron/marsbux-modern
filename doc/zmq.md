...
The marsbux Core daemon can be configured to act as a trusted "border
router", implementing the marsbux wire protocol and relay, making
...
The ZeroMQ feature in marsbux Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building marsbuxd:
...
For instance:
    ...
    $ marsbuxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/marsbuxd.tx.raw \
...
These options can also be provided in marsbux.conf.
...
From the perspective of marsbuxd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into marsbuxd directly. Furthermore, no information is
...
using. marsbuxd appends an up-counting sequence number to each
