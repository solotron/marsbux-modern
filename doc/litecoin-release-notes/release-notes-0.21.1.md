...
Summary:
==============
In this update to marsbux Core, Taproot has been activated, enhancing marsbux’s security, privacy and scalability. marsbux Core 0.21.1 is the most significant update in years - with performance improvements, new features (like Taproot), bug fixes and updated translations - and sets the foundations for MWEB’s imminent arrival. 

marsbux Core 0.21.1 is available now, right here: <https://download.marsbux.org/marsbux-0.21.1/>.

As always, we welcome the expertise of the community. To report bugs please use the issue tracker at GitHub - <https://github.com/marsbux-project/marsbux/issues > - and to get security and update notifications subscribe via <https://groups.google.com/forum/#!forum/marsbux-dev>. 

Anyone can contribute to marsbux Core. Please scroll down to ‘How to contribute to marsbux Core’ for details.

By downloading or upgrading marsbux Core, you’re helping to secure the network, so thank you and enjoy the upgrade!


How to upgrade: 
==============

Firstly, thank you for running marsbux Core and helping secure the network!

As you’re running an older version of marsbux Core, shut it down. Wait until it’s completely shut down  - which might take a few minutes for older versions - then follow these simple steps:
For Windows: simply run the installer 
For Mac: copy over to `/Applications/marsbux-Qt` 
For Linux: copy cover `marsbuxd`/`marsbux-qt`.

NB: upgrading directly from an ‘end of life’ version of marsbux Core is possible, but it might take a while if the data directory needs to be migrated. Old wallet versions of marsbux Core are generally supported.
 

Compatibility:
==============

marsbux Core is supported and extensively tested on operating systems using the Linux kernel, macOS 10.10+,  Windows 7 and newer. It’s not recommended to use marsbux Core on unsupported systems.

marsbux Core should also work on most other Unix-like systems, but is not as frequently tested on them.

From marsbux Core 0.21.1 onwards, macOS versions earlier than 10.12 are no longer supported. Additionally, marsbux Core doesn’t yet change appearance when macOS "dark mode" is activated.
