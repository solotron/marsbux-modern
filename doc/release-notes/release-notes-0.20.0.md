...
marsbux Core version 0.20.0 is now available from:

  <https://marsbuxcore.org/bin/marsbux-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  marsbux
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use marsbux Core on
unsupported systems.

From marsbux Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, marsbux Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `marsbux_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from marsbux Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since marsbux Core version 0.18.0.
...
- Testing or debugging of implementations of the marsbux P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of marsbux Core. marsbux Core logs debug messages
...
- OpenSSL is no longer used by marsbux Core.  (#17265)

- BIP70 support has been fully removed from marsbux Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/marsbux/marsbux/issues/16599),
  [PR #16702](https://github.com/marsbux/marsbux/pull/16702), and the
  `marsbuxd help` for more information.  This option is experimental and
...
- All custom settings configured when marsbux Core starts are now
...
- The "Start marsbux Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://marsbuxcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple marsbux Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed marsbux Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/marsbux-core/HWI) for
  signing.  Future versions of marsbux Core should support a GUI option
...
- marsbux Core's automatically-generated source code documentation is
  now available at https://doxygen.marsbuxcore.org.  (#17596)
...
- The `marsbux-cli` utility used with the `-getinfo` parameter now
...
- The `marsbux-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `marsbuxstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_marsbux-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBmarsbux_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_marsbux` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBmarsbux_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for marsbux-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate marsbux schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest marsbux.conf (sdaftuar)
...
- #18022 Fix appveyor `test_marsbux` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (marsbux-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone marsbux-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about marsbux Core files (hebasto)
...
- #17119 Fix broken marsbux-cli examples (andrewtoth)
...
- #17503 Remove bitness from marsbux-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.marsbuxcore.org (laanwj)
...
- #17648 Rename wallet-tool references to marsbux-wallet (hel-o)
...
- #16115 On marsbuxd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from marsbux-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/marsbux/marsbux/).
